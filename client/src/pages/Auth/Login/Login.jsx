...
const Login = ({sidebar}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post('https://tourismhost-b1r6.vercel.app/api/auth/login', input);
      console.log(data);
      if (data?.success) {
  ...
  const [text] = useTypewriter({
    words: ['Beautiful Destinations', 'Travelling India.', 'Login to access more features'],
    loop: {},
  });
     ...
     <Sidebar sidebar={sidebar}/>
      <div className="content">
        ...
        <form onSubmit={handleSubmit}>
          <div className="form">
            <input type="email" className='.inp' placeholder="Email Address" name="email" value={input.email} onChange={handleInput} required/>

            {showPassword && (
              <input className='.inp' type="password" placeholder="Password" name="password" value={input.password} onChange={handleInput} required/>
            )}
